...
const replyBtn = async id => {
  const msg = await articleStore.createArticleComment(+route.params.id, replyContent[id], id);
  if (msg) return Notification.error("回复失败");
  await articleStore.getArticleComment(+route.params.id);
  // 回复评论，且获取到评论后，隐藏回复区域
...
const deleteOk = async () => {
  visible.value = false;
  const msg = await articleStore.removeArticleComment(curCommentId.value);
